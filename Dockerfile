FROM nvcr.io/nvidia/modulus/modulus:24.01 as builder
...
# COPY ml-workspace/resources/libraries ${RESOURCES_PATH}/libraries
...
FROM nvcr.io/nvidia/modulus/modulus:24.01 as final
...
COPY --from=builder $CONDA_ROOT/bin/conda $CONDA_ROOT/bin/conda
COPY --from=builder $HOME/ $HOME/
COPY --from=builder /tini /tini
COPY --from=builder /etc/jupyter /etc/jupyter
COPY --from=builder /usr/bin/clean-layer.sh /usr/bin/clean-layer.sh
COPY --from=builder /usr/bin/fix-permissions.sh /usr/bin/fix-permissions.sh
...
COPY --from=builder /usr/local/nvm/versions/node/v16.20.2 /usr/local/nvm/versions/node/v16.20.2
